...
def collate_fn_new(batch_data):
    N = len(batch_data)
    compound_max = 100
    num_feature = 7
    compound_new = torch.zeros((N, compound_max),dtype=torch.long)
    feature_new = torch.zeros((N, num_feature), dtype=torch.float32)
    labels_new = torch.zeros(N, dtype=torch.float32)
    
    for i,pair in enumerate(batch_data):
        pair = pair.strip().split(',')
        '''
        label : target (hyperparameter)
        '''
        if hp.target == 'HLM':
            compoundstr, label, features = pair[1], pair[3], pair[4:]
        elif hp.target == 'MLM':
            compoundstr, label, features = pair[1], pair[2], pair[4:]
        # print(len(features)) # 7
        # compoundstr, label, features = pair[0], pair[1], pair[2:]
        # print(pair)
        # print(dd)
        ...
        feature_array = np.array([float(x) if x else 0.0 for x in features], dtype=np.float32)
        feature_tensor = torch.tensor(feature_array)
        feature_new[i] = feature_tensor
        
        label = float(label)
        label_np = np.float32(label)
        label_tensor = torch.tensor(label_np, dtype=torch.float32)
        labels_new[i] = label_tensor
        
    return (compound_new, feature_new, labels_new)

def collate_fn_new_dacon(batch_data):
    N = len(batch_data)
    compound_max = 100
    num_feature = 7
    compound_new = torch.zeros((N, compound_max),dtype=torch.long)
    ...
    for i,pair in enumerate(batch_data):
        ...
        label : target (hyperparameter)
        '''
        compoundstr, features = pair[1], pair[2:]
        # print(pair)
        # print(dd)
    ...
    return (compound_new, feature_new)
